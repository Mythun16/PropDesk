  ...
  return (
    <div className="page-container">
      <div className="page-header-row">
        <h1 className="page-title" style={{ margin: 0 }}>Portal Posts</h1>
      ...
      {posts.length === 0 ? (
        <div className="empty-state">
          <Globe size={48} />
          <p>No portal posts yet.</p>
          <button className="btn-primary" onClick={() => setShowModal(true)}>Create First Post</button>
        </div>
      ) : (
                ...
                return (
                  <tr key={post.id}>
                    <td style={{ fontWeight: 600, fontSize: '0.85rem' }}>
                      <div>{post.property_title || (prop ? (prop.title || prop.location_str) : null) || post.property_id.slice(-6)}</div>
                      {post.agent_name && (
                        <div style={{ fontSize: '0.72rem', color: 'var(--muted-text)', marginTop: '0.15rem' }}>
                          {post.agent_name}{post.agent_phone ? ` · ${post.agent_phone}` : ''}
                        </div>
                      )}
                    </td>
                    <td>
                    ...
                    <td style={{ fontSize: '0.8rem', color: 'var(--muted-text)' }}>
                      {post.portal_listing_id || '—'}
                    </td>
                    <td style={{ fontSize: '0.8rem', color: 'var(--muted-text)' }}>{formatDate(post.posted_at)}</td>
                    <td style={{ fontSize: '0.8rem', color: 'var(--muted-text)' }}>{formatDate(post.created_at)}</td>
                    <td>
      ...
      {showModal && (
        <div className="modal-overlay" onClick={() => setShowModal(false)}>
          <div className="modal-content" style={{ maxWidth: 440 }} onClick={e => e.stopPropagation()}>
            <h3 className="modal-title">New Portal Post</h3>
              ...
              <select className="form-select" value={form.property_id} onChange={e => setForm(p => ({ ...p, property_id: e.target.value }))}>
                ...
                {properties.filter(p => p.status === 'available').map(p => (
                  <option key={p.id} value={p.id}>
                    {p.title || p.location_str || `#${String(p.set_no).padStart(3, '0')}`} — {p.district || p.location?.city}
                  </option>
                ))}
              </select>
            </div>
